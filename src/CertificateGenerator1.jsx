...
export default function CertificateGenerator1() {
  ...
  const drawCertificate = (text, dpr = window.devicePixelRatio || 1) => {
    const canvas = canvasRef.current;
    const ctx = canvas.getContext("2d");
    ctx.setTransform(1, 0, 0, 1, 0, 0); // reset transform trước khi scale
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ...
    ctx.fillText(displayName, baseWidth / 2, 1700);
    ...
    const subText = `Nhân viên ưu tú nhất quý ${quarter} năm ${year}`.toUpperCase();
